  ...
  return (
    <>
      ...
      <div className={`fixed top-0 right-0 bg-white text-dark-text w-full max-w-[500px] h-full z-30 p-4 transition-all ease-in-out duration-300 ${animate ? 'translate-x-0 opacity-100' : 'translate-x-[50px] opacity-0 '}`}>
        ...
        {/* no notification */}
        <div className="h-full w-full flex items-start justify-start px-2 py-8 flex-col gap-2 overflow-y-auto">
          <div className="w-full min-h-[140px] flex items-start justify-start gap-5">
            <div className="h-full w-[50%] flex flex-col relative">
              <h1 className="text-sm capitalize min-h-fit mb-2">From: {openedJourney.from}</h1>
              <img src={imageFrom.image} className="w-full h-full max-h-[130px] min-h-[130px] rounded-xl shadow-md object-cover object-bottom" />
              <a href={`https://www.google.com/maps/place/${openedJourney.from}`} target="_blank" className="group p-2 rounded-xl bg-white/90 transition active:scale-105 backdrop-blur-sm shadow-lg absolute bottom-2 left-2">
            ...
            <div className="h-full w-[50%] flex flex-col relative">
              ...
              <img src={imageTo.image} className="w-full h-full max-h-[130px] min-h-[130px] rounded-xl shadow-md object-cover object-bottom"/>
              <a href={`https://www.google.com/maps/place/${openedJourney.to}`} target="_blank" className="group p-2 rounded-xl bg-white/90 transition active:scale-105 backdrop-blur-sm shadow-lg absolute bottom-2 left-2">
              <FaMapLocationDot className="text-sm transition group-hover:text-main-color" />
              </a>
            </div>
          </div>
          <span className="border-b-[1px] border-border-lines-light my-3 w-full max-w-[70%] mx-auto h-[1px]" ></span>
